...
$c_sql_acesso = "select usuarios.tipo, perfil_usuarios.indicadores_comparativos FROM usuarios
...
if ($registro_acesso['tipo'] == 'Operador' && $registro_acesso['indicadores_comparativos'] == 'N') {
    header('location: /gop/acesso.php');
}
$_SESSION['titulo_rel'] = "Relatório Comparativo Mensal de Centros de Custo da Manutenção por Período";
$_SESSION['titulo_graf'] = "Gráfico Comparativo Mensal de Centros de Custo da Manutenção por Período";
...
if ((isset($_POST["btnpesquisa"])) && ($_SERVER['REQUEST_METHOD'] == 'POST')) {
    ...
    $c_centrocusto = $_POST["centrocusto"];
    $c_sql_centrocusto = "select centrodecusto.id, centrodecusto.descricao from centrodecusto where centrodecusto.descricao = '$c_centrocusto'";
    //echo $c_sql_centrocusto;
    $result = $conection->query($c_sql_centrocusto);
    $c_linha = $result->fetch_assoc();
    $i_id_centrocusto = $c_linha['id'];
    $c_where = $c_where . "setores.id_centrodecusto='$i_id_centrocusto' and  ";
    $c_query = $c_query . 'Centro de Custo:' . $c_linha['descricao'] . '-';

    $c_where = $c_where = substr($c_where, 0, -5); // tirar o and no final
    // montagem do sql para recursos físicos
    //
    $c_sql = "SELECT extract(month FROM ordens.data_geracao) AS mes, setores.id_centrodecusto, centrodecusto.descricao, COUNT(setores.id_centrodecusto) total
            FROM ordens 
            JOIN setores ON ordens.id_setor=setores.id
            JOIN centrodecusto ON setores.id_centrodecusto=centrodecusto.id
            where $c_where 
            GROUP BY extract(month FROM ordens.data_geracao), setores.id_centrodecusto ORDER BY mes";
...
<body>
    <div class="panel panel-primary class">
        <div class="panel-heading text-center">
            ...
            <h5>Opções para Relatório Mensais para Centro de Custos<h5>
        </div>
    </div>
