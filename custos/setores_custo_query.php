...
if ((isset($_POST["btnpesquisa"])) && ($_SERVER['REQUEST_METHOD'] == 'POST')) {
    ...
    //
    $c_sql = "SELECT ordens.id_setor, setores.descricao, sum(ordens.valor_material) AS total_material,
            SUM(ordens.valor_servico) AS total_servico
            FROM ordens 
            JOIN setores ON ordens.id_setor=setores.id
            where ordens.`status`='C' and $c_where GROUP BY ordens.id_setor";
    ...
    //echo $c_sql;
    echo "<script> window.open('/gop/custos/setores_custo_relatorio.php?id=', '_blank');</script>";
}
...
<body>
    <div class="panel panel-primary class">
        <div class="panel-heading text-center">
            ...
            <h5>Opções para Relatório de custos por Setores<h5>
        </div>
    </div>
